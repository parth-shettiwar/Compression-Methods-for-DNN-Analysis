...
parser.add_argument('--model_dir', default='experiments/cnn_distill3', help="Directory of params.json")
...
def model_analysis(model, dataloader, params, temperature=1., num_classes=10):
    ...
    with tqdm(total=len(dataloader)) as t:
        for idx, (data_batch, labels_batch) in enumerate(dataloader):
            ...
            t.update()
            accr += np.sum(predict_correct_batch)
            den_accr += len(labels_batch)
            tot_time += end-start
            tot_images += len(output_batch)
    
    softmax_scores = np.vstack(softmax_scores)
    predict_correct = np.vstack(predict_correct)
    print("Inference Time taken: ", tot_time)
    print("Inference Time taken per image: ", tot_time/tot_images)
    print("Accuracy: ",accr/den_accr)
...
if __name__ == '__main__':
    ...
    model = net.Net3(params).cuda() if params.cuda else net.Net(params)
